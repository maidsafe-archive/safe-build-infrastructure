...
function create_security_groups() {
    ...
    echo "Creating linux_slaves-dev security group."
    response=$(aws ec2 create-security-group \
        --region eu-west-2 \
        --group-name linux_slaves-dev \
        --description "Ports required for Linux slaves" \
        --vpc-id vpc-f124b999)
    linux_group_id=$(jq '.GroupId' <<< "$response" | sed 's/\"//g')
    echo "$linux_group_id" > .aws_provision/linux_security_group_id
...
function open_ports() {
    ...
    do
        echo "Opening port $port on linux slaves group."
        aws ec2 authorize-security-group-ingress \
            --region eu-west-2 \
            --group-id "$linux_group_id" \
            --protocol tcp \
            --port "$port" \
            --cidr 0.0.0.0/0
    done
