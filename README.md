...
We are using the [GitHub OAuth Plugin](https://wiki.jenkins.io/display/JENKINS/GitHub+OAuth+Plugin) for authentication with Jenkins. This needs to be configured manually after first login. Go to Manage Jenkins -> Configure Global Security -> Security Realm. Change this to 'GitHub Authentication Plugin', then provide the client ID and secret; they can both be found in the QA Keypass database. Now scroll down to the authorization section. Matrix-based security should already be specified, so now you need to click on the 'Add user or group...' button and provide `maidsafe*QA` as the value. Tick the Overall/Administer and Overall/Read checkboxes for that user. After this, click on the Save button. This should log you out, and if you refresh the page you should get prompted to authorize with GitHub.
